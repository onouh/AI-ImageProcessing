...
WEIGHTS_URL = "https://download.pytorch.org/models/convnext_small-0c510722.pth"
...
class ConvNeXtSmall(nn.Module):
    ...
    def __init__(self, pretrained: bool = True) -> None:
        ...
        for stage_idx, (depth, dim) in enumerate(
            ...
            for _ in range(depth):
                sd = sd_prob_max * block_idx / (total_blocks - 1)
    ...
    def _load_pretrained(self) -> None:
        ...
        if missing:
            print(f"[ConvNeXtSmall] Missing keys: {missing}")
        if unexpected:
            print(f"[ConvNeXtSmall] Unexpected keys (ignored): {unexpected}")
